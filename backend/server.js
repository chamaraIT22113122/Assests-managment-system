...
// ─────────────────────────────────────────────
// IoT WebSocket Registry
// ─────────────────────────────────────────────
io.on('connection', (socket) => {
  console.log('IoT Device connected:', socket.id);
  socket.on('iot_health_ping', async (payload) => {
    try {
      await db(
      ...
      io.emit('dashboard_update', { type: 'IOT_UPDATE', payload });
    } catch (err) {
...
// ─────────────────────────────────────────────
// Cron: License Expiry Check
// ─────────────────────────────────────────────
cron.schedule('0 0 * * *', async () => {
  console.log('[Worker] Checking for expiring licenses...');
  // Add license table logic here if needed
});
...
server.listen(PORT, () => {
  ...
  console.log(`☁️  Connected to Neon Postgres (Free Tier) - data persists forever!`);
});
