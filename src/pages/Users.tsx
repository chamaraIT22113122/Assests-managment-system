    ...
    <div className="py-8">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Manage members and assign them to companies. Members log in using their Employee ID only.</p>
        </div>
        {canEdit && (
          <button 
            onClick={() => openModal()}
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-100 dark:border-slate-700">
              <th className="py-3 px-4 font-semibold text-sm text-slate-600 dark:text-slate-300">Member Name</th>
              <th className="py-3 px-4 font-semibold text-sm text-slate-600 dark:text-slate-300">Company ID</th>
          ...
          <tbody>
              ...
              <tr key={u._id} className="border-b border-slate-50 dark:border-slate-700/50 hover:bg-blue-50/50 dark:hover:bg-blue-900/10 transition-colors group cursor-pointer"
                ...
                <td className="py-3 px-4">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-[#3b5998] flex items-center justify-center text-white text-sm font-bold shrink-0">
                      {u.name?.charAt(0).toUpperCase()}
                    </div>
                    <span className="text-sm font-semibold text-slate-800 dark:text-slate-200 group-hover:text-[#3b5998] dark:group-hover:text-blue-400 transition-colors">{u.name}</span>
                  </div>
                </td>
                <td className="py-3 px-4 text-sm font-mono text-[#3b5998] dark:text-blue-400">{u.companyId}</td>
                <td className="py-3 px-4 text-sm text-slate-500 dark:text-slate-400">{u.companyName || '—'}</td>
                <td className="py-3 px-4">
                  <div className="flex justify-end gap-1" onClick={e => e.stopPropagation()}>
                    <button onClick={() => navigate(`/admin/users/${u.id || u._id}`)} title="View Profile"
                      className="p-1.5 text-[#3b5998] hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-md transition-colors"><Eye size={16} /></button>
                    {canEdit && (
                      <>
                        <button onClick={() => openModal(u)} title="Edit"
                          className="p-1.5 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-md transition-colors"><Edit size={16} /></button>
            ...
            {users.length === 0 && (
              <tr>
                <td colSpan={4} className="py-8 text-center text-slate-500 dark:text-slate-400">No members found.</td>
              </tr>
            )}
          </tbody>
        </table>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-md shadow-xl overflow-hidden">
            <div className="flex justify-between items-center p-6 border-b border-slate-100 dark:border-slate-700">
              <h2 className="text-xl font-bold text-slate-800 dark:text-white">{editingId ? 'Edit Member' : 'Add Member'}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input required type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg dark:bg-slate-900 dark:text-white outline-none focus:border-[#3b5998]" placeholder="e.g. John Doe" />
              </div>
              ...
              <div>
                ...
                <select 
                  ...
                  className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg dark:bg-slate-900 dark:text-white outline-none focus:border-[#3b5998]"
                >
              ...
              <div>
                ...
                <input required type="text" value={formData.companyId} onChange={(e) => setFormData({...formData, companyId: e.target.value})} className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg dark:bg-slate-900 dark:text-white outline-none focus:border-[#3b5998] font-mono" placeholder="e.g. BIT-10008" />
                <p className="text-xs text-slate-500 mt-1">🔑 Members use this ID to log into the employee portal. No password needed.</p>
              </div>
              ...
              <div className="pt-4 flex justify-end gap-3">
                ...
                <button type="submit" disabled={companies.length === 0} className="px-4 py-2 bg-[#3b5998] hover:bg-blue-700 text-white font-medium rounded-lg transition-colors disabled:opacity-50">
