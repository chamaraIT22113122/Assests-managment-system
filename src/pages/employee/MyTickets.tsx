...
const IT_CATEGORIES = [
  ...
  { label: 'Other / General', icon: HelpCircle, color: 'text-slate-700', description: 'Anything else not listed above' },
];
...
const STATUS_COLORS: Record<string, string> = {
  ...
  'Closed': 'bg-slate-100 text-slate-700 dark:bg-[#000000] dark:text-slate-400 border-slate-400',
};
  ...
  return (
    <div className={`mt-4 rounded-xl border p-4 ${isComplete ? 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800/50' : 'bg-blue-50 dark:bg-blue-900/10 border-blue-200 dark:border-blue-800/40'}`}>
        ...
        {MAINT_STAGES.map((s, i) => (
          <React.Fragment key={s}>
            <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full border transition-all ${
              ...
              'bg-[#e5e4e2] dark:bg-[#000000] text-slate-800 border-slate-400 dark:border-slate-700'
            }`}>
              {STAGE_ICONS[s]} {s}
            </span>
      ...
      <div className="flex items-center gap-2">
        <div className="flex-1 h-2 bg-[#e5e4e2] dark:bg-slate-700 rounded-full border border-slate-400 dark:border-slate-600 overflow-hidden">
      ...
      {(job.assigned_to || job.notes?.length > 0) && (
        <div className="mt-3 pt-3 border-t border-blue-100 dark:border-blue-800/40 space-y-1">
          {job.assigned_to && (
            <p className="text-xs text-slate-800 dark:text-slate-400">👷 Assigned to: <strong>{job.assigned_to}</strong></p>
          )}
          {job.notes?.length > 0 && (
            <div className="text-xs text-slate-800 dark:text-slate-400 bg-[#e5e4e2] dark:bg-[#000000]/60 rounded-lg px-3 py-2 mt-1 border border-slate-300 dark:border-slate-700">
              <span className="font-semibold">Latest update:</span> {job.notes[job.notes.length - 1]?.text}
              <span className="text-slate-800 ml-1">— {job.notes[job.notes.length - 1]?.by}</span>
            </div>
          )}
        </div>
      )}
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#e5e4e2] dark:bg-[#000000] rounded-2xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[90vh]">
        <div className="flex justify-between items-center p-6 border-b border-slate-300 dark:border-slate-700 shrink-0">
          <div>
            <h2 className="text-xl font-bold text-[#000000] dark:text-[#e5e4e2]">{step === 1 ? 'What do you need help with?' : 'Raise IT Support Ticket'}</h2>
            <p className="text-sm text-slate-700 mt-0.5">{step === 1 ? 'Select the type of IT issue you are facing.' : `Category: ${form.category}`}</p>
          </div>
          <button onClick={onClose} className="text-slate-800 hover:text-slate-700 dark:hover:text-white"><X size={22} /></button>
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {IT_CATEGORIES.map(cat => (
                <button key={cat.label} onClick={() => handleCategorySelect(cat.label)}
                  className="flex items-start gap-4 p-4 rounded-xl border border-slate-400 dark:border-slate-700 hover:border-[#fcaf17] hover:bg-blue-50 dark:hover:bg-blue-900/10 transition-all text-left group">
                  <div className={`mt-0.5 ${cat.color}`}><cat.icon size={22} /></div>
                  <div>
                    <p className="font-semibold text-[#000000] dark:text-[#e5e4e2] text-sm group-hover:text-[#fcaf17] dark:group-hover:text-blue-400 transition-colors">{cat.label}</p>
                    <p className="text-xs text-slate-700 mt-0.5 leading-relaxed">{cat.description}</p>
                  </div>
                </button>
              ))}
            </div>
            ...
            <form id="ticket-form" onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <input required value={form.title} onChange={e => setForm({...form, title: e.target.value})}
                  className="w-full px-4 py-2.5 border border-slate-400 dark:border-slate-700 rounded-xl dark:bg-[#000000] dark:text-[#e5e4e2] outline-none focus:border-[#fcaf17]"
              ...
              <div>
                ...
                <textarea required value={form.description} onChange={e => setForm({...form, description: e.target.value})} rows={4}
                  className="w-full px-4 py-2.5 border border-slate-400 dark:border-slate-700 rounded-xl dark:bg-[#000000] dark:text-[#e5e4e2] outline-none focus:border-[#fcaf17] resize-none"
                  placeholder="Describe the problem in detail..." />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">How did it happen? <span className="text-slate-800 font-normal">(optional)</span></label>
              ...
              {(form.priority === 'High' || form.priority === 'Critical') && (
                <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 rounded-xl p-4">
                  ...
                  <textarea required value={form.urgencyReason} onChange={e => setForm({...form, urgencyReason: e.target.value})} rows={2}
                    className="w-full px-4 py-2.5 border border-red-200 rounded-xl bg-[#e5e4e2] dark:bg-[#000000] dark:text-[#e5e4e2] outline-none resize-none" />
                </div>
              )}
            </form>
          )}
        </div>
        <div className="p-6 border-t border-slate-300 dark:border-slate-700 shrink-0 flex justify-between items-center bg-[#e5e4e2] dark:bg-[#000000]/80">
          <button type="button" onClick={step === 1 ? onClose : () => setStep(1)}
            className="px-4 py-2 text-slate-800 dark:text-slate-300 font-medium hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl transition-colors">
            {step === 1 ? 'Cancel' : '← Back'}
          </button>
          {step === 2 && (
            <button type="submit" form="ticket-form" disabled={loading}
              className="px-6 py-2.5 bg-[#fcaf17] hover:bg-blue-700 disabled:opacity-70 text-white font-semibold rounded-xl transition-colors">
  ...
  return (
    <div className="py-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-[#000000] dark:text-[#e5e4e2]">My Tickets</h1>
          <p className="text-sm text-slate-700 dark:text-slate-400 mt-1">Track the status of your IT support requests.</p>
        </div>
        <button onClick={() => setShowModal(true)}
          className="bg-[#fcaf17] hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl font-semibold flex items-center gap-2 transition-colors shadow-md">
      ...
      <div className="bg-[#e5e4e2] dark:bg-[#000000] rounded-2xl shadow-md border border-slate-300 dark:border-slate-700 overflow-hidden">
        {loading ? (
          ...
          <div className="p-16 text-center flex flex-col items-center">
            ...
            <h3 className="text-xl font-bold text-[#000000] dark:text-[#e5e4e2] mb-2">No tickets yet</h3>
            <p className="text-slate-700 max-w-sm mb-6">Having an IT issue? Raise a ticket and our IT team will get back to you.</p>
            <button onClick={() => setShowModal(true)} className="bg-[#fcaf17] hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-semibold transition-colors">
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-700/50">
            {tickets.map(ticket => (
              <div key={ticket._id} className="p-5 hover:bg-[#e5e4e2] dark:hover:bg-slate-700/20 transition-colors">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 cursor-pointer"
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <span className="text-xs font-mono font-bold text-slate-800 bg-slate-100 dark:bg-[#000000] px-2 py-0.5 rounded">
                    ...
                    <h3 className="font-semibold text-[#000000] dark:text-[#e5e4e2] truncate">{ticket.title}</h3>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <ChevronDown size={16} className={`text-slate-800 transition-transform ${expandedId === ticket._id ? 'rotate-180' : ''}`} />
                  </div>
                </div>

                {expandedId === ticket._id && (
                  <div className="mt-4 pt-4 border-t border-slate-300 dark:border-slate-700">
                    <div className="bg-[#e5e4e2] dark:bg-[#000000]/50 rounded-xl p-4 mb-4 text-sm text-slate-700 dark:text-slate-300 whitespace-pre-wrap">
                      {ticket.description}
                    </div>
                        ...
                        <div className="space-y-3">
                          {ticket.history.map((item: any, idx: number) => (
                            <div key={idx} className="flex gap-3 text-sm relative">
                              <div className="w-2.5 h-2.5 rounded-full bg-[#fcaf17] mt-1 shrink-0 z-10" />
                              ...
                              <div>
                                ...
                                {item.note && <p className="text-slate-800 dark:text-slate-400 mt-0.5">{item.note}</p>}
                                <span className="text-xs text-slate-800">{new Date(item.date).toLocaleString()}</span>
                              </div>
                            </div>
                          ))}
                        </div>
