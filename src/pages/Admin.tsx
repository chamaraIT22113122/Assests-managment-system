...
const ACCESS_LEVELS = [
  ...
  { value: 'none', label: 'No Access',   description: 'Hidden from admin', color: 'text-slate-500 dark:text-slate-400', bg: 'bg-slate-100 dark:bg-slate-700 border-slate-200 dark:border-slate-600', icon: <Ban size={13} /> },
];
...
const AdminModal = ({ admin, onClose, onSaved }: any) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[92vh]">
        {/* Header */}
        <div className="flex justify-between items-center p-6 border-b border-slate-100 dark:border-slate-700 shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#3b5998] flex items-center justify-center">
              <Shield size={20} className="text-white" />
            </div>
            <div>
              <h2 className="text-lg font-bold text-slate-800 dark:text-white">{isEdit ? 'Edit Admin Account' : 'Create Admin Account'}</h2>
              <p className="text-xs text-slate-500">Set credentials and section-level access permissions</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto">
          <div className="p-6 space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })}
                  required={!isEdit}
                  className="w-full pl-10 pr-4 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl dark:bg-slate-900 dark:text-white outline-none focus:border-[#3b5998] text-sm"
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-800 dark:text-white mb-3 flex items-center gap-2">
                <Shield size={15} className="text-[#3b5998]" /> Section Access Permissions
              </h3>
              <div className="space-y-2 bg-slate-50 dark:bg-slate-900/50 rounded-xl p-4 border border-slate-100 dark:border-slate-700">
                {SECTIONS.map(sec => (
          ...
          <div className="p-6 border-t border-slate-100 dark:border-slate-700 flex justify-end gap-3 shrink-0 bg-slate-50 dark:bg-slate-800/80">
            <button type="button" onClick={onClose} className="px-4 py-2 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl text-sm font-medium">Cancel</button>
    ...
    <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-md shadow-2xl p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className="text-lg font-bold text-slate-800 dark:text-white">Delete Admin Account</h2>
        ...
        <div className="mb-5">
          ...
          <input value={confirm} onChange={e => setConfirm(e.target.value)}
            className="w-full px-4 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl dark:bg-slate-900 dark:text-white outline-none focus:border-red-500 text-sm font-mono"
            placeholder="Type DELETE" />
        </div>

        <div className="flex justify-end gap-3">
          <button onClick={onClose} className="px-4 py-2 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl text-sm font-medium">Cancel</button>
          <button
...
const Admin = () => {
  ...
  return (
    <div className="py-8 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
            <Shield size={22} className="text-[#3b5998]" /> Admin Controls
          </h1>
      ...
      {/* Legend */}
      <div className="flex flex-wrap gap-3 p-4 bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 items-center">
        <span className="text-xs font-bold text-slate-500 dark:text-slate-400 mr-1">Permission Levels:</span>
        {ACCESS_LEVELS.map(al => (
          ...
          <table className="w-full text-left border-collapse min-w-max">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-700/40 border-b border-slate-100 dark:border-slate-700">
                <th className="py-3 px-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider sticky left-0 bg-slate-50 dark:bg-slate-700/40 z-10 min-w-[180px]">Admin</th>
                <th className="py-3 px-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider min-w-[170px]">Email / Username</th>
                {SECTIONS.map(s => (
                  <th key={s.key} className="py-3 px-2 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-center whitespace-nowrap min-w-[110px]">
                    {s.icon} {s.label}
                  </th>
                ))}
                <th className="py-3 px-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-right sticky right-0 bg-slate-50 dark:bg-slate-700/40 z-10">Actions</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={SECTIONS.length + 3} className="py-12 text-center text-slate-500">
                ...
                return (
                  <tr key={admin._id}
                    className="border-b border-slate-50 dark:border-slate-700/40 hover:bg-slate-50 dark:hover:bg-slate-700/20 transition-colors">

                    {/* Name */}
                    <td className="py-3 px-4 sticky left-0 bg-white dark:bg-slate-800 z-10">
                      <div className="flex items-center gap-3">
                        <div className={`w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm shrink-0 ${isMaster ? 'bg-amber-400 text-white' : 'bg-[#3b5998] text-white'}`}>
                          {isMaster ? <Crown size={16} /> : admin.name.charAt(0).toUpperCase()}
                        </div>
                        <div>
                          <p className="text-sm font-semibold text-slate-800 dark:text-white leading-tight">{admin.name}</p>
                          {isMaster
                    ...
                    {/* Email */}
                    <td className="py-3 px-4 text-sm text-slate-600 dark:text-slate-400">{admin.email || '—'}</td>
                    ...
                    {/* Actions */}
                    <td className="py-3 px-4 sticky right-0 bg-white dark:bg-slate-800 z-10">
