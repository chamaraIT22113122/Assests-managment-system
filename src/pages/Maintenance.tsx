...
// ── Workflow Stages ───────────────────────────────────────
const STAGES = [
  { key: 'Received',     label: 'Received',       icon: '📥', color: 'bg-[#e5e4e2]0',  light: 'bg-slate-100 text-slate-700 border-slate-400 dark:bg-slate-700 dark:text-slate-300' },
  ...
  return (
    <div className="flex items-center gap-2 mt-1">
      <div className="flex-1 h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
        <div className="h-full bg-[#fcaf17] rounded-full transition-all duration-500" style={{ width: `${pct}%` }} />
      </div>
        ...
        <React.Fragment key={s.key}>
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all ${
              ...
              'bg-[#e5e4e2] dark:bg-slate-700 text-slate-800 border-slate-400 dark:border-slate-600 hover:border-slate-400'
            }`}
          >
            {s.icon} {s.label}
          </button>
      ...
      <div className="bg-[#e5e4e2] dark:bg-[#000000] rounded-2xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex justify-between items-start p-6 border-b border-slate-300 dark:border-slate-700 shrink-0">
          <div>
            ...
            <h2 className="text-lg font-bold text-[#000000] dark:text-[#e5e4e2]">{job.title}</h2>
            {job.asset_name && (
              <p className="text-sm text-slate-700 mt-0.5">🖥️ {job.asset_name} {job.asset_code && `(${job.asset_code})`}</p>
            )}
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          {/* Progress Stepper */}
          <div>
            <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider mb-3">Processing Stage</h3>
            {canEdit ? (
          ...
          {canEdit ? (
            <div>
              <label className="text-xs font-bold text-slate-800 uppercase tracking-wider block mb-2">Assigned Technician</label>
            ...
            <div>
              <label className="text-xs font-bold text-slate-800 uppercase tracking-wider block mb-2">Assigned Technician</label>
              <div className="w-full px-4 py-2.5 border border-slate-400 dark:border-slate-700 rounded-xl bg-[#e5e4e2] dark:bg-[#000000] dark:text-slate-300 text-sm">
          ...
          {job.notes?.length > 0 && (
            <div>
              <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider mb-3">Progress Notes</h3>
              <div className="space-y-2">
                {job.notes.map((n: any, i: number) => (
                  <div key={i} className="flex gap-3 text-sm">
                    <div className="w-2 h-2 rounded-full bg-[#fcaf17] mt-1.5 shrink-0" />
                    <div className="flex-1 bg-[#e5e4e2] dark:bg-[#000000]/50 rounded-xl px-3 py-2 border border-slate-300 dark:border-slate-700">
                      <p className="text-slate-700 dark:text-slate-300">{n.text}</p>
                      <p className="text-xs text-slate-800 mt-1">{n.by} · {new Date(n.date).toLocaleString()}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {canEdit && (
            <div>
              <label className="text-xs font-bold text-slate-800 uppercase tracking-wider block mb-2">Add Progress Note</label>
              <textarea value={note} onChange={e => setNote(e.target.value)} rows={3}
                className="w-full px-4 py-2.5 border border-slate-400 dark:border-slate-700 rounded-xl dark:bg-[#000000] dark:text-[#e5e4e2] outline-none focus:border-[#fcaf17] text-sm resize-none"
                placeholder="e.g. Replaced faulty RAM module. Testing now..." />
            </div>
          )}
        </div>

        <div className="p-6 border-t border-slate-300 dark:border-slate-700 bg-[#e5e4e2] dark:bg-[#000000]/80 shrink-0 flex justify-end gap-3">
          <button onClick={onClose} className="px-4 py-2 text-slate-800 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl text-sm font-medium transition-colors">Close</button>
          {canEdit && (
            <button onClick={handleSave} disabled={saving}
              className="px-5 py-2 bg-[#fcaf17] hover:bg-blue-700 text-white rounded-xl text-sm font-semibold transition-colors disabled:opacity-60">
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#e5e4e2] dark:bg-[#000000] rounded-2xl w-full max-w-lg shadow-2xl">
        <div className="flex justify-between items-center p-6 border-b border-slate-300 dark:border-slate-700">
          <h2 className="text-lg font-bold text-[#000000] dark:text-[#e5e4e2]">🔧 Create Maintenance Job</h2>
          <button onClick={onClose} className="text-slate-800 hover:text-slate-700 dark:hover:text-white"><X size={20} /></button>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input value={form.assetCode} onChange={e => setForm({...form, assetCode: e.target.value})}
                className="w-full px-4 py-2.5 border border-slate-400 dark:border-slate-700 rounded-xl dark:bg-[#000000] dark:text-[#e5e4e2] outline-none focus:border-[#fcaf17] text-sm font-mono"
    ...
    <div className="space-y-6 py-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[#000000] dark:text-[#e5e4e2] flex items-center gap-2">
            <Wrench size={24} className="text-[#fcaf17]" /> Maintenance
          </h1>
          <p className="text-sm text-slate-700 dark:text-slate-400 mt-1">Track and manage all IT hardware maintenance jobs.</p>
        </div>
        {canEdit && (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        {stageCounts.map(s => (
          <button key={s.key}
            ...
            className={`rounded-xl p-4 border text-left transition-all ${filterStage === s.key ? `${s.light} ring-2 ring-current` : 'bg-[#e5e4e2] dark:bg-[#000000] border-slate-300 dark:border-slate-700 hover:border-slate-300'}`}>
            <p className="text-2xl font-bold text-[#000000] dark:text-[#e5e4e2]">{s.count}</p>
            <p className="text-xs font-semibold text-slate-700 dark:text-slate-400 mt-0.5">{s.icon} {s.label}</p>
          </button>
        ))}
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          <Search size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-800" />
        ...
        <select value={filterStage} onChange={e => setFilterStage(e.target.value)}
          className="w-full sm:w-auto px-3 py-2.5 border border-slate-400 dark:border-slate-700 rounded-xl dark:bg-[#000000] dark:text-[#e5e4e2] outline-none text-sm focus:border-[#fcaf17]">
      ...
      <div className="bg-[#e5e4e2] dark:bg-[#000000] rounded-2xl border border-slate-300 dark:border-slate-700 shadow-md overflow-hidden">
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="font-semibold text-slate-700">No maintenance jobs found</p>
            <p className="text-sm text-slate-800 mt-1">Create a new job or escalate from a support ticket.</p>
          </div>
        ) : (
            ...
            {filtered.map(job => {
              ...
              return (
                <motion.div key={job._id} layout
                  className="p-5 hover:bg-[#e5e4e2] dark:hover:bg-slate-700/20 transition-colors cursor-pointer group"
                  onClick={() => setSelectedJob(job)}>
                  <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                    {/* Left info */}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <h3 className="font-semibold text-[#000000] dark:text-[#e5e4e2] text-sm group-hover:text-[#fcaf17] transition-colors">{job.title}</h3>
                        {job.ticket_id && (
                      ...
                      <div className="flex flex-wrap items-center gap-3 text-xs text-slate-700 dark:text-slate-400">
                    ...
                    <div className="flex items-center gap-2 shrink-0" onClick={e => e.stopPropagation()}>
                      ...
                      {canEdit && stageIdx < STAGES.length - 1 && (
                        <button
                          ...
                          className="px-2.5 py-1 bg-[#fcaf17] hover:bg-blue-700 text-white text-xs font-bold rounded-lg transition-colors flex items-center gap-1">
        ...
        {!loading && filtered.length > 0 && (
          <div className="px-5 py-3 border-t border-slate-300 dark:border-slate-700 text-xs text-slate-800">
