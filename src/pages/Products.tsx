...
const Products = () => {
  ...
  const renderIcon = (iconName: string) => {
    ...
    return <IconComponent size={20} className="text-[#3b5998] dark:text-blue-400" />;
  };
  ...
  return (
    <div className="py-8">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Asset Categories</h1>
          <p className="text-slate-500 text-sm mt-1">Manage the categories that appear on the Asset Dashboard.</p>
        </div>
        {canEdit && (
          <button 
            onClick={() => openModal()}
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-100 dark:border-slate-700">
              <th className="py-3 px-4 font-semibold text-sm text-slate-600 dark:text-slate-300 w-16">Icon</th>
              <th className="py-3 px-4 font-semibold text-sm text-slate-600 dark:text-slate-300">Category Name</th>
              {canEdit && <th className="py-3 px-4 font-semibold text-sm text-slate-600 dark:text-slate-300 text-right">Actions</th>}
            </tr>
          </thead>
          <tbody>
            {categories.map((category) => (
              <tr key={category._id} className="border-b border-slate-50 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                <td className="py-3 px-4">
                  <div className="w-10 h-10 rounded-lg bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center">
                    {renderIcon(category.icon)}
                  </div>
                </td>
                <td className="py-3 px-4 text-sm font-medium text-slate-800 dark:text-slate-200">{category.name}</td>
                {canEdit && (
            ...
            {categories.length === 0 && (
              <tr>
                <td colSpan={canEdit ? 3 : 2} className="py-8 text-center text-slate-500 dark:text-slate-400">No categories found.</td>
              </tr>
            )}
          </tbody>
        </table>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-md shadow-xl overflow-hidden">
            <div className="flex justify-between items-center p-6 border-b border-slate-100 dark:border-slate-700">
              <h2 className="text-xl font-bold text-slate-800 dark:text-white">{editingId ? 'Edit Category' : 'Add Category'}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input required type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg dark:bg-slate-900 dark:text-white outline-none focus:border-[#3b5998]" placeholder="e.g. Desktop" />
              </div>
              <div>
                  ...
                  {ICONS_LIST.map(iconName => {
                    ...
                    return (
                      <button
                        ...
                        className={`p-3 rounded-xl flex items-center justify-center transition-all ${isSelected ? 'bg-[#3b5998] text-white shadow-md scale-105' : 'bg-slate-50 dark:bg-slate-700 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-600'}`}
                      >
                        <IconComp size={20} />
                      </button>
                    )
