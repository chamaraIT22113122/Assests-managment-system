  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Assets Inventory</h1>
          <p className="text-slate-500 text-sm mt-1">Manage and track all company hardware and equipment.</p>
        </div>
        {canEdit && (
          <button onClick={() => openModal()} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2.5 rounded-xl text-sm font-medium transition-colors shadow-sm flex items-center gap-2">
            <Plus size={18} />
            Add Asset
          </button>
        )}
      ...
      >
        {/* ... Search ... */}
        <div className="p-4 border-b border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row sm:items-center gap-4 justify-between bg-slate-50/50 dark:bg-slate-800/50">
          <div className="relative w-full sm:w-80">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
            <input 
              type="text" 
              placeholder="Search by name, ID, or user..." 
              className="w-full pl-10 pr-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:border-blue-400 focus:ring-4 focus:ring-blue-100 dark:focus:ring-blue-900/20 outline-none transition-all dark:text-white"
            />
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700 text-sm text-slate-500 dark:text-slate-400">
                <th className="px-6 py-4 font-medium">Asset ID & Name</th>
                <th className="px-6 py-4 font-medium">Category</th>
                <th className="px-6 py-4 font-medium">Company</th>
                <th className="px-6 py-4 font-medium">Assigned To</th>
                <th className="px-6 py-4 font-medium">Status</th>
                <th className="px-6 py-4 font-medium">Warranty Ends</th>
                <th className="px-6 py-4 font-medium text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              {assets.map((asset) => (
                <tr key={asset._id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-slate-800 dark:text-white text-sm transition-colors cursor-pointer" onClick={() => openDetails(asset)}>{asset.name}</p>
                        <p className="text-xs text-slate-500 dark:text-slate-400 font-mono">{asset.assetCode}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-300">{asset.category}</td>
                  <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-300">{asset.company || '-'}</td>
                  <td className="px-6 py-4">
                    {!asset.assignedTo ? (
                      <span className="text-slate-400 text-sm italic">Unassigned</span>
                    ) : (
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded-full bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 flex items-center justify-center text-xs font-bold uppercase">
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border ${
                      ...
                      asset.status === 'Retired' ? 'bg-slate-100 text-slate-600 border-slate-300 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700' :
                  ...
                  <td className="px-6 py-4 text-right flex justify-end gap-1">
                    <button onClick={() => openDetails(asset)} className="p-1.5 text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-700 rounded-md transition-colors" title="View Details"><Eye size={16} /></button>
                    {canEdit && (
                      <>
                        <button onClick={() => openModal(asset)} className="p-1.5 text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-md transition-colors" title="Edit"><Edit size={16} /></button>
                        <button onClick={() => handleDelete(asset._id)} className="p-1.5 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-md transition-colors" title="Delete"><Trash2 size={16} /></button>
                      </>
                    )}
                  </td>
                </tr>
              ...
              {assets.length === 0 && (
                <tr>
                  <td colSpan={7} className="py-8 text-center text-slate-500 dark:text-slate-400">No assets found.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        {isDetailsOpen && selectedAsset && (
          ...
          >
            <motion.div 
              ...
              className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-4xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"
            >
              <div className="flex justify-between items-center p-6 border-b border-slate-100 dark:border-slate-700 shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400 font-mono">{selectedAsset.assetCode}</p>
                  </div>
                </div>
                <button onClick={() => setIsDetailsOpen(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors"><X size={24} /></button>
              </div>

              <div className="flex flex-col md:flex-row overflow-y-auto custom-scrollbar flex-1">
                {/* Left Column: Asset Info & QR */}
                <div className="md:w-1/3 p-6 bg-slate-50/50 dark:bg-slate-800/50 border-r border-slate-100 dark:border-slate-700">
                  <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col items-center text-center mb-6">
                    <div className="bg-white p-3 rounded-lg shadow-sm border border-slate-100 mb-4 inline-flex flex-col items-center">
                      <QRCodeCanvas 
                        id="asset-qr-code"
                    ...
                    <div className="flex gap-2 w-full mt-2 mb-4">
                      <button onClick={downloadQR} className="flex-1 px-3 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-semibold rounded-lg transition-colors">
                        Download
                      </button>
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">Scan for asset details</p>
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 break-all">{selectedAsset.serialNumber}</p>
                  </div>
                      ...
                      <div className="space-y-3">
                        <div className="flex gap-3 items-start">
                          ...
                          <div>
                            ...
                            <p className="text-sm font-medium text-slate-800 dark:text-slate-200">{selectedAsset.category}</p>
                          </div>
                        </div>
                        <div className="flex gap-3 items-start">
                          <User size={16} className="text-slate-400 mt-0.5" />
                          <div>
                            <p className="text-xs text-slate-500">Owner Company</p>
                            <p className="text-sm font-medium text-slate-800 dark:text-slate-200">{selectedAsset.company || 'N/A'}</p>
                          </div>
                        </div>
                        <div className="flex gap-3 items-start">
                          <Calendar size={16} className="text-slate-400 mt-0.5" />
                          <div>
                            <p className="text-xs text-slate-500">Warranty Range</p>
                    ...
                    {(!selectedAsset.history || selectedAsset.history.length === 0) ? (
                      ...
                      [...selectedAsset.history].reverse().map((record: any, idx: number) => (
                        <div key={idx} className="relative">
                          <div className="absolute -left-[33px] top-1 w-4 h-4 rounded-full bg-white dark:bg-slate-800 border-2 border-blue-500 ring-4 ring-white dark:ring-slate-800"></div>
                          <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-xl border border-slate-100 dark:border-slate-700">
                            ...
                            <p className="text-sm text-slate-800 dark:text-slate-200 font-medium mb-1">{record.note}</p>
                            <p className="text-xs text-slate-500">By: {record.user}</p>
                          </div>
                        </div>
                      ))
                    )}
        ...
        {isModalOpen && canEdit && (
          ...
          >
            <motion.div 
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-2xl shadow-xl overflow-hidden max-h-[90vh] flex flex-col"
            >
              <div className="flex justify-between items-center p-6 border-b border-slate-100 dark:border-slate-700 shrink-0">
                <h2 className="text-xl font-bold text-slate-800 dark:text-white">{editingId ? 'Edit Asset' : 'Add Asset'}</h2>
                ...
                <form id="asset-form" onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="col-span-1 md:col-span-2">
                      ...
                      <input required type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg dark:bg-slate-900 dark:text-white outline-none focus:border-blue-500" placeholder="e.g. Dell XPS 15 9500" />
                    </div>
                    
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Product Category</label>
                      <select required value={formData.category} onChange={(e) => setFormData({...formData, category: e.target.value})} className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg dark:bg-slate-900 dark:text-white outline-none focus:border-blue-500">
                    ...
                    <div>
                      ...
                      <input required type="text" value={formData.serialNumber} onChange={(e) => setFormData({...formData, serialNumber: e.target.value})} className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg dark:bg-slate-900 dark:text-white outline-none focus:border-blue-500" placeholder="SN-XXXX-YYYY" />
                    </div>
                  </div>

                  {/* Assignment Info */}
                  <div className="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Company</label>
                      <select required value={formData.company} onChange={handleCompanyChange} className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg dark:bg-slate-900 dark:text-white outline-none focus:border-blue-500">
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg dark:bg-slate-900 dark:text-white outline-none focus:border-blue-500"
                      />
                      ...
                      {isMemberDropdownOpen && (
                        <div className="absolute z-10 mt-1 w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                          <button
                            ...
                            className="w-full text-left px-4 py-2 text-sm text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700/50 italic border-b border-slate-100 dark:border-slate-700"
                          >
                            Unassigned
                          </button>
                          {filteredUsers.length > 0 ? (
                            filteredUsers.map(u => (
                              ...
                              >
                                ...
                                <span className="text-xs text-slate-500 dark:text-slate-400 font-mono">{u.companyId}</span>
                              </button>
                            ))
                          ) : (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input type="date" value={formData.warrantyEnd} onChange={(e) => setFormData({...formData, warrantyEnd: e.target.value})} className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg dark:bg-slate-900 dark:text-white outline-none focus:border-blue-500" />
                    </div>
                    
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Physical Location</label>
                      <input type="text" value={formData.location} onChange={(e) => setFormData({...formData, location: e.target.value})} className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg dark:bg-slate-900 dark:text-white outline-none focus:border-blue-500" placeholder="e.g. Server Room A" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Condition</label>
                      <select required value={formData.status} onChange={(e) => setFormData({...formData, status: e.target.value})} className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg dark:bg-slate-900 dark:text-white outline-none focus:border-blue-500">
              ...
              <div className="p-6 border-t border-slate-100 dark:border-slate-700 shrink-0 bg-slate-50 dark:bg-slate-800 flex justify-end gap-3">
                <button type="button" onClick={closeModal} className="px-4 py-2 text-slate-600 dark:text-slate-300 font-medium hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg transition-colors">Cancel</button>
                <button type="submit" form="asset-form" className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors shadow-sm">
