    ...
    <div className="w-full">
      {isDropdown ? (
        <button 
          ...
          className={`w-full flex items-center justify-between px-4 py-2.5 rounded-lg text-sm font-medium transition-colors text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800`}
        >
        ...
        <NavLink 
          ...
          onClick={onClick}
          className={({ isActive }) => `w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors ${isActive ? 'bg-[#3b5998] text-white' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 dark:hover:text-white'}`}
        >
          <Icon size={18} />
          <span>{label}</span>
        </NavLink>
...
const Sidebar = ({ isMobileMenuOpen, setIsMobileMenuOpen }: any) => {
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col shadow-xl md:shadow-none md:relative transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
      <div className="flex items-center justify-between mt-6 mb-8 px-6">
        <div className="flex items-center gap-2 text-[#3b5998] dark:text-blue-400">
          <Briefcase size={24} className="fill-current" />
          <h1 className="text-xl font-bold tracking-wider">ASSETTO</h1>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={toggleTheme} className="hidden md:block text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors">
            {theme === 'light' ? <Moon size={18} /> : <Sun size={18} />}
          </button>
          <button onClick={closeMenu} className="md:hidden text-slate-400 hover:text-slate-600 dark:hover:text-white p-1">
            <X size={24} />
          </button>
        </div>
      </div>

      <div className="mb-2 px-6 text-xs font-semibold text-slate-400 uppercase tracking-wider">Menu</div>
      ...
      <div className="mt-4 py-4 px-4 border-t border-slate-100 dark:border-slate-800">
        ...
        <button
          ...
          className="w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium text-slate-600 dark:text-slate-400 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-600 dark:hover:text-red-400 transition-colors group"
        >
